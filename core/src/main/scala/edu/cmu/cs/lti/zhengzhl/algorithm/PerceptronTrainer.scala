...
class PerceptronTrainer(tagNames:Array[String]) extends Trainer(tagNames){
  ...
  def updateWeight(key:String, change:Double) {
   ...
   def onlineTraining(sentence : List[Token]) {
      ...
      sentence.zipWithIndex.zip(decodedTags).foreach{case ((token,index),decodedTag) =>{
        if (token.ner != decodedTag){
          ...
          fDecode.foreach(decodeFeature =>{
            if (!fTrue.contains(decodeFeature)){
//              println("- Feature fired by decoded but not real : %s".format(decodeFeature))
              updateWeight(decodeFeature,-stepSize)
            }
          })

          fTrue.foreach(tureFeature =>{
            if (!fDecode.contains(tureFeature)){
//              println("+ Feature fired by real but not decode : %s".format(tureFeature))

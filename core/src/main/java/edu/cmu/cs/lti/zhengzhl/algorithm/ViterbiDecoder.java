...
//	private String sentenceBoundary;
	...
	public ViterbiDecoder(String[] stateSet, String sentenceStart) {
		this.stateSet = stateSet;

		this.sentenceStart = sentenceStart;
	}

	public String[] decode(Token[] observations) {
	...
	private void viterbi(Token[] observations, double[][] lattice, int[][] backPointers) {
		...
		for (int col = 0; col < observations.length; col++) {
			...
			for (int stateIndex = 0; stateIndex < stateSet.length; stateIndex++) {
				...
				if (col == 0) {
					...
					lattice[col][stateIndex] = getScore(observations, col, sentenceStart, currentState);
	...
	protected abstract double getScore(Token[] observations, int index, String previousState, String currentState);
}
